...
export default function SubscriptionManager({ currentFarm }) {
  ...
  // Determine the current subscription based on the first item in the subscriptions array
  // This maintains compatibility with the existing component logic that expects a single subscription object.
  ...
  const loadData = async () => {
    if (!currentFarm) return; // Ensure currentFarm exists before fetching
    setIsLoading(true);
    try {
      const [subs, pays] = await Promise.all([
        Subscription.filter({ farm_id: currentFarm.id }),
        Payment.filter({ farm_id: currentFarm.id }, "-payment_date"),
      ]);
      setSubscriptions(Array.isArray(subs) ? subs : []);
      setPayments(Array.isArray(pays) ? pays : []);
    } catch (error) {
      console.error("Error loading data:", error);
      toast({ title: "שגיאה", description: "טעינת פרטי המנוי והתשלומים נכשלה", variant: "destructive" });
    }
    setIsLoading(false);
  };
  ...
  const handleUpgrade = async (newPlanType) => {
    try {
      ...
      if (subscription) { // Use the derived current subscription
        await Subscription.update(subscription.id, updatedSubscriptionData);
      } else {
  ...
  const handlePayment = async (paymentData) => {
    try {
      if (!subscription) {
        toast({ title: "שגיאה", description: "לא קיים מנוי פעיל לביצוע תשלום עבורו.", variant: "destructive" });
        return;
      }
      // כאן יהיה האינטגרציה עם ספק התשלומים
      const payment = await Payment.create({
        farm_id: currentFarm.id,
        subscription_id: subscription.id, // Use the derived current subscription
      ...
      await Subscription.update(subscription.id, { // Use the derived current subscription
        status: 'active',
